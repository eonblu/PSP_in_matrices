...
def AdjustedFindHorizontalPivot(matrix, CompsObj):
    ...
    for rem in range(len(matrix)):
        if not rem in ignored_rows: # if row was removed in earlier step, ignore
            ...
            setof_q_r.append(select_kth_triplet(R, s, CompsObj)) # math.ceil(s/2)
    p = setof_q_r[0]
...
def AdjustedFindVerticalPivot(matrix, CompsObj):
    t = 0
    ignored_columns = set()
    rows_to_remove = []
    s = 7
    ...
    for rem in range(len(matrix[0])):
        if not rem in ignored_columns:
            ...
            setof_q_r.append(select_kth_triplet(R, 1, CompsObj)) # math.ceil(s/2)
    p = setof_q_r[0]
...
def AdjustedReduceMatrix(matrix, CompsObj):
...
def AdjustedReduceMatrixTestsuite(matrix, CompsObj):
    p_hor, columns_to_remove = AdjustedFindHorizontalPivot(matrix, CompsObj)
    p_ver, rows_to_remove = AdjustedFindVerticalPivot(matrix, CompsObj)
