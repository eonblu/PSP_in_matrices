...
def SAdjustedFindHorizontalPivot(matrix, CompsObj):
    ...
    columns_to_remove = [] 
    # Phase 1
    while len(matrix) - len(ignored_rows) > math.floor(len(matrix)**(1/2)):
        setof_q_i = []
...
def SAdjustedFindVerticalPivot(matrix, CompsObj):
    t = 0
    ignored_columns = set()
    rows_to_remove = []
    s = 7
    # Phase 1
    while len(matrix[0]) - len(ignored_columns) > math.floor(len(matrix[0])**(1/2)):
        setof_q_i = []
...
def SAdjustedReduceMatrix(matrix, CompsObj):
...
def SAdjustedReduceMatrixTestsuite(matrix, CompsObj):
    p_hor, columns_to_remove = SAdjustedFindHorizontalPivot(matrix, CompsObj)
    p_ver, rows_to_remove = SAdjustedFindVerticalPivot(matrix, CompsObj)
